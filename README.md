...
WebRTC（Web Real-Time Communication）是一套开放的浏览器实时通信技术与标准，让浏览器在不依赖插件的情况下，直接进行音视频通话、屏幕共享、以及点对点数据传输。

## 主要应用场景

- **视频会议**：腾讯会议、Zoom 等企业协作和在线教育
...
- **完全锥型 NAT（Full Cone）**：最宽松，一旦建立内网端口映射，任何外部主机都可以使用 NAT 设备的公网 IP 和端口与内部主机通信
- **受限锥型 NAT（Restricted Cone）**：只允许之前内部发起连接时的目标 IP 发送数据回来
- **端口受限锥型 NAT（Port Restricted Cone）**：更严格，除了限制 IP，还限制端口
- **对称型 NAT（Symmetric）**：最严格且最难穿透，为每个外部目标分配一个独立的公网端口，只允许该外部目标通过该特定端口进行通信
...
> ⚠️ 部分：其中一方是对称型 NAT，在 UDP 打洞时，成功率不稳定，表现为“有时能穿透、有时失败”。
>
> 不同厂商的 NAT 设备对 RFC 标准的实现存在差异，有些实现并不完全严格。例如，端口分配算法可能非完全随机、某些设备在短时间内可能会保留相同的端口映射等。
...
假设客户端 A 和客户端 B 都使用对称型 NAT，尝试建立 P2P 连接：
...
ICE 是一种用于在复杂网络环境中建立点对点连接的框架协议。它通过收集多种候选连接地址，并系统地测试这些候选地址对之间的连通性，从而找到最优的通信路径。ICE 结合了 STUN 和 TURN 协议，能够智能地穿透各种类型的 NAT 和防火墙，优先选择直连方式以降低延迟，在直连失败时自动降级使用中继服务器，最终确保 WebRTC 等实时通信应用能够在互联网上可靠地建立连接。
...
1. **收集候选地址（ICE Candidates）**
   ...
   - **主机候选（Host Candidates）**：本地网络接口地址，通常是局域网 IP 地址，如 192.168.0.100
   - **服务器反射候选（Server Reflexive Candidates）**：通过 STUN 服务器获取的公网地址
   - **中继候选（Relay Candidates）**：通过 TURN 服务器中继的地址
...
STUN 是一种轻量级的网络协议，它的主要作用是帮助位于 NAT 后面的设备发现自己的公网 IP 地址和端口映射关系。当客户端向 STUN 服务器发送请求时，服务器会将客户端的公网地址信息返回给它，使得客户端能够了解自己在互联网上的真实可达地址，从而可以将这个地址信息告知对等端，实现点对点的直接通信。
...
SFU 是多人实时音视频通信中最常见的一种服务端媒体处理架构。你可以把它理解成一个智能转发站：每个客户端都把自己的音视频流发送给 SFU，SFU 不负责重新编码或混流，而是根据订阅关系、网络状况和会议布局，将对应的媒体流转发给其他参会者。
...
SFU 服务器收到的是 A、B、C 各自独立的音视频流，转发客户端通常是同时收到多路流，然后由浏览器或应用自己决定如何渲染这些画面。
...
- **服务端压力相对可控**：因为主要是转发，不做复杂的音视频合成
- **客户端需要处理多路流**：参会人数越多，终端解码和渲染压力越大
- **画质和布局更灵活**：
  - 可以选择只订阅特定用户的音视频，从而减少当前接收端不必要的下行带宽和解码消耗
  - 可以让缩略图使用更低分辨率或更低码率，把清晰度优先留给主画面
  - 可以根据用户当前的网络状况动态切换码率或视频层级，尽可能保证通话流畅性

**优点**

- 延迟较低，适合实时互动场景
- 服务端不需要对每路流重新编码，成本比 MCU 低
- 每路流独立，便于做大小流、码率自适应、发言人切换
- 某个用户网络较差时，可以只降低该用户订阅的质量，而不是影响所有人

**缺点**

- 客户端要同时接收和解码多路流，终端压力较大
- 会议人数很多时，下行带宽消耗明显
- 布局组合通常在客户端完成，前端实现会更复杂
...
目前大多数 WebRTC 视频会议系统，都是基于 SFU 架构实现的，因为它在实时性、成本以及可扩展性之间取得了较好的平衡。

### MCU（Multipoint Control Unit）多点控制单元

MCU 也是一种用于多人实时音视频通信的服务端媒体处理架构。与 SFU 不同，MCU 不只是转发，而是会在服务端对多路音视频进行**解码、处理、合成，重新编码**，最后把合成后的单路音视频流发送给每个客户端。

**数据流向**

```
用户 A ─┐
用户 B ─┼→ MCU（解码 → 混音/合图 → 重新编码）→ 单路流发给每个用户
用户 C ─┘
```

例如，一个 4 人会议，在 MCU 模式下：

- 每个人把自己的流上传给 MCU
- MCU 再把 4 路视频合成一个画面，把多路音频混成一路
- 每个客户端通常只接收一路合成后的画面

这意味着：

- **客户端压力更小**：因为通常只需要解码一路流
- **服务端压力很大**：因为要做解码、混音、合图、编码
- **灵活性较弱**：客户端拿到的是已经混好的结果，不容易对单个用户单独控制

**优点**

- 客户端实现简单，终端解码压力小，兼容性更好
- 下行带宽消耗更稳定，尤其适合弱终端设备
- 服务端更容易做统一媒体处理，例如，统一降噪、音量均衡、加水印、转码成指定分辨率或码率后再分发

**缺点**

- 服务端 CPU 和带宽成本高
- 编解码、合成会导致更高延迟
- 单个用户的画质、布局、订阅策略不如 SFU 灵活
- 扩展到大规模会议时成本通常更高

**适用场景**

- 终端性能较弱、希望降低客户端解码压力的场景
- 需要服务端统一控制会议布局和最终画面的场景
- 需要服务端直接输出录制流、直播流等合成结果的场景

**SFU 与 MCU 的核心区别**

- **SFU**：服务器主要负责转发，每个客户端接收多路独立流
- **MCU**：服务器负责把多路流合成为一路，客户端通常只接收一路流

### SDP（Session Description Protocol）会话描述协议

SDP 是一种基于文本的协议格式，用于描述多媒体会话的各种参数和能力信息。

在 WebRTC 中，SDP 充当通信双方协商媒体传输细节的“说明书”，它包含了媒体描述（音频、视频）以及数据通道相关的协商信息、编解码器及其参数、网络传输地址（IP 和端口）、带宽要求、加密方式等关键信息。

在双方通信时，发起方创建 SDP Offer 描述自己支持的媒体能力和偏好，接收方返回 SDP Answer 表明自己的能力并选择双方都支持的配置，这种 Offer/Answer 模型使得两端能够就如何建立媒体流达成一致。虽然 SDP 格式较为复杂且不易阅读，但它是 WebRTC 信令交换的核心载体，确保了不同设备、浏览器之间能够准确理解对方的媒体能力并建立兼容的通信会话。
...
- 双方通过某种方式（如房间号、用户 ID）表明希望与哪个对等端通信
...
- 这个交换过程持续进行，直到双方都收集完所有候选（当收到空候选）
...
UDP 是一种无连接的传输层协议。凭借其无连接、低延迟的特性，成为 WebRTC 底层的首选传输协议。
...
- **消息边界**：像 UDP 一样保留消息边界，能够清楚地区分每条消息的开始和结束，使得传输结构化数据更加方便
...
1. **基于丢包的控制**（Loss-based）
   ...
   - 监测 RTCP 报告中的丢包率
   - 如果丢包率升高 → 可能是拥塞或网络质量差 → 降低码率
   - 如果丢包率低 → 说明网络良好 → 尝试提高码率

2. **基于延迟的控制**（Delay-based）

   - 通过接收端的到达时间间隔变化，计算延迟梯度
   - 如果延迟增加 → 网络队列积压 → 提前降低码率（有拥塞迹象）
   - 如果延迟稳定 → 网络畅通 → 可以提高码率
   - **优势**：延迟增加是拥塞的早期信号，当网络开始拥塞时，数据包会在路由器队列中排队等待，导致延迟上升。此时丢包可能还未发生，但如果不及时降低码率，队列溢出后就会开始大量丢包。因此，基于延迟的控制介入更早，能在丢包前预判拥塞与提前干预。
...
- **快速降低**：检测到拥塞立即降低码率（0.8-0.85 倍），避免雪崩
- **谨慎增加**：网络改善时缓慢提升码率（1.03-1.08 倍），避免再次拥塞
- **持续监测**：每秒评估多次，保持快速响应
...
当出现网络抖动时，数据包到达的时间间隔不规律，序号顺序错乱。此时，先放入缓冲区等待排序。
...
- **自适应调整**：WebRTC 持续估算网络抖动情况，动态缩放缓冲区深度，而不是使用固定大小。
- **与 NACK 的协同**：当收到乱序包时，Jitter Buffer 模块检测到序列号空洞（gap），会先在缓冲区内等待一段时间（包可能晚到）。若超时仍未到达，则由 RTCP 模块发出 NACK 请求发送方重传。重传包及时到达则无缝补上，否则触发丢包补偿：
  - 音频：PLC（Packet Loss Concealment），由 Opus 等解码器插值生成近似音频，掩盖丢包
  - 视频：冻结上一帧显示（画面不黑屏、不花屏），或发送 PLI（Picture Loss Indication），请求发送方编码器生成新的关键帧
- **指标监控**：可通过 `RTCPeerConnection.getStats()` 获取 `jitterBufferDelay`、`jitterBufferEmittedCount` 等指标，用于诊断缓冲区健康状况和播放有无延迟。
...
- SFU 架构框架（[LiveKit](https://github.com/livekit/livekit/)、[mediasoup](https://github.com/versatica/mediasoup/)）
- TURN 服务器搭建（[coturn](https://github.com/coturn/coturn/)）
